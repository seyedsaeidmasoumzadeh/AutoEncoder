...
class AutoEncoder(object):
    def __init__(self):
        ...
        def encoder(x):
            ...
            out_do1_reshaped = tf.reshape(out_do1, shape=[-1, 25 * 25 * 25])
            # ------------ Fully Connected Layer #1 -------------
            input_size = out_do1_reshaped.shape[1:]
            input_size = int(np.prod(input_size))
            W_fc1 = tf.get_variable(name='w_fc1', shape=[input_size, 25 * 25 * 5],
                                    initializer=tf.contrib.layers.xavier_initializer(uniform=False))
            b_fc1 = tf.get_variable(name='b_fc1', shape=[25 * 25 * 5],
                                    initializer=tf.contrib.layers.xavier_initializer(uniform=False))
            ...
            input_size = int(np.prod(input_size))
            W_fc2 = tf.get_variable(name='W_fc2', shape=[input_size, 25 * 25 ],
                                    initializer=tf.contrib.layers.xavier_initializer(uniform=False))
            b_fc2 = tf.get_variable(name='b_fc2', shape=[25 * 25],
                                    initializer=tf.contrib.layers.xavier_initializer(uniform=False))
        ...
        def decoder(x):
            ...
            input_size = int(np.prod(input_size))
            W_fc3 = tf.get_variable(name='W_fc3', shape=[input_size, 25 * 25 * 5],
                                    initializer=tf.contrib.layers.xavier_initializer(uniform=False))
            b_fc3 = tf.get_variable(name='b_fc3', shape=[25 * 25 * 5],
                                    initializer=tf.contrib.layers.xavier_initializer(uniform=False))
            ...
            input_size = int(np.prod(input_size))
            W_fc4 = tf.get_variable(name='W_fc4', shape=[input_size, 25 * 25 * 25],
                                    initializer=tf.contrib.layers.xavier_initializer(uniform=False))
            b_fc4 = tf.get_variable(name='b_fc4', shape=[25 * 25 * 25],
                                    initializer=tf.contrib.layers.xavier_initializer(uniform=False))
            ...
            out_do4 = tf.reshape(out_do4, shape=[-1, 25, 25, 25])
            ...
            input_size = int(np.prod(input_size))
            W_fc5 = tf.get_variable(name='W_fc5', shape=[input_size, 50 * 50 * 3],
                                    initializer=tf.contrib.layers.xavier_initializer(uniform=False))
            b_fc5 = tf.get_variable(name='b_fc5', shape=[50 * 50 * 3],
                                    initializer=tf.contrib.layers.xavier_initializer(uniform=False))
        ...
        # Construct model
        input = tf.reshape(self.X, shape=[-1, 50, 50, 3])
